...
class GraphUpsampling(nn.Module):
    ...
    # def GAMMAPARAM(self, U, A, gamma=0.5, method=WEI, gamma_learn=False):
    #     # NOTE: gamma = 1 is equivalent to no_A
    #     super(GraphUpsampling, self).__init__()
    #     if A is not None:
    #         assert np.allclose(A, A.T), 'A should be symmetric'
    ...
    #             self.gamma = gamma
    #         if method in [BIN, WEI]:
    #             # COMPROBAR QUE EL VALOR DE A Y DE GAMMA CAMBIAN AL OPTIMIZARLO
    #             # INTENTAR QUE EL VALOR DE GAMMA SE QUEDE ENTRE 0 y 1
    #             self.A = self.gamma*np.eye(A.shape[0]) + (1-self.gamma)*self.A
    #         self.A = Tensor(self.A)
