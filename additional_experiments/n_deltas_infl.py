...
if __name__ == '__main__':
    ...
    mean_err = np.zeros((Gs['n_graphs'], N_EXPS, len(DELTAS)))
    median_err = np.zeros((Gs['n_graphs'], N_EXPS, len(DELTAS)))
    node_err = np.zeros((Gs['n_graphs'], N_EXPS, len(DELTAS)))
    for i, deltas in enumerate(DELTAS):
        print('P_N:', deltas)
        with Pool(processes=N_CPUS) as pool:
            results = []
            for j in range(Gs['n_graphs']):
                results.append(pool.apply_async(run,
                               args=[j, Gs, signals, learning, deltas]))
            for j in range(Gs['n_graphs']):
                mean_err[j, :, i], median_err[j, :, i], node_err[j, :, i] = \
                    results[j].get()

        # Print summary
        utils.print_partial_results(deltas, EXPS, node_err[:, :, i],
                                    median_err[:, :, i])
    ...
    end_time = time.time()
    utils.print_results(DELTAS, EXPS, node_err, median_err)
