...
def train_test_split(data, labels, train_test_coef):
    ndata = data.shape[0]
    train_test_idx = int(train_test_coef * ndata)
    train_data = data[0:train_test_idx,...]
    test_data = data[train_test_idx:,...]
    train_labels = labels[0:train_test_idx,...]
    test_labels = labels[train_test_idx:,...]
    return train_data, train_labels, test_data, test_labels

def generate_labels_GIGOMax(data, mapping, nodes_com):
    """
    Generate the labels for the GIGO graph as the max of each community
    """
    nsamples = data.shape[0]
    N_comm = len(nodes_com)
    labels = np.zeros((nsamples, N_comm))
    for t in range(nsamples):
        for c in range(N_comm):
            labels[t,c] = np.max(data[t,nodes_com[c]])
    return labels

"""
Code below has been kindly provided by Fernando Gama
"""
